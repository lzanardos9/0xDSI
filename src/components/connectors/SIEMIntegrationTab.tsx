...
const PLATFORM_COLORS: Record<string, { bg: string; text: string; border: string; gradient: string }> = {
  'Splunk': { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/30', gradient: 'from-emerald-500 to-green-600' },
  'QRadar': { bg: 'bg-blue-500/10', text: 'text-blue-400', border: 'border-blue-500/30', gradient: 'from-blue-500 to-blue-700' },
  'Sentinel': { bg: 'bg-cyan-500/10', text: 'text-cyan-400', border: 'border-cyan-500/30', gradient: 'from-cyan-500 to-teal-600' },
  'Elastic': { bg: 'bg-amber-500/10', text: 'text-amber-400', border: 'border-amber-500/30', gradient: 'from-amber-500 to-orange-600' },
};
...
export default function SIEMIntegrationTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-slate-800 to-emerald-900/50 rounded-xl p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 mt-1 text-sm">Bi-directional integration with enterprise SIEM platforms</p>
          </div>
      ...
      <div className="flex gap-3">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
            !selectedPlatform ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/30' : 'text-slate-400 hover:bg-slate-800/50 border border-slate-700/50'
          }`}
        >
          All Platforms
        </button>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                selectedPlatform === p ? `${cfg.bg} ${cfg.text} border ${cfg.border}` : 'text-slate-400 hover:bg-slate-800/50 border border-slate-700/50'
              }`}
            >
            ...
            <div key={conn.id} className={`bg-slate-800/40 rounded-xl border ${cfg.border} p-5 transition-all hover:bg-slate-800/60`}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-400">{conn.integration_type} -- v{conn.version}</div>
                  </div>
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-xs font-medium ${conn.status === 'active' ? 'text-emerald-400' : 'text-amber-400'}`}>
                    {conn.status}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                {[
                  ...
                  <div key={label} className="bg-slate-900/50 rounded-lg p-2.5 text-center border border-slate-700/50">
              ...
              <div className="grid grid-cols-4 gap-2 text-xs">
                <div className="text-center">
                  ...
                  <div className={`font-bold ${conn.queue_depth > 5000 ? 'text-red-400' : 'text-slate-200'}`}>{formatNumber(conn.queue_depth)}</div>
                </div>
                <div className="text-center">
                  <div className="text-slate-500">Uptime</div>
                  <div className="font-bold text-emerald-400">{conn.uptime_percent}%</div>
                </div>
                <div className="text-center">
                  <div className="text-slate-500">Sources</div>
                  <div className="font-bold text-slate-200">{conn.log_sources}</div>
                </div>
                <div className="text-center">
                  <div className="text-slate-500">Parsers</div>
                  <div className="font-bold text-slate-200">{conn.parsers_active}</div>
                </div>
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-slate-700/50 flex items-center justify-between">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${cfg.bg} ${cfg.text} border ${cfg.border}`}>{conn.platform}</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="bg-slate-800/40 rounded-xl border border-slate-700/50 overflow-hidden">
        ...
        <div className="divide-y divide-slate-700/30">
          {FORWARDING_RULES.map((rule) => (
            <div key={rule.id} className="p-4 hover:bg-slate-700/20 transition-colors">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                    rule.priority === 1 ? 'bg-red-500/10 text-red-400 border border-red-500/30' : rule.priority === 2 ? 'bg-amber-500/10 text-amber-400 border border-amber-500/30' : 'bg-slate-700/50 text-slate-400 border border-slate-600'
                  }`}>
                    P{rule.priority}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <div className="text-sm font-bold text-white">{formatNumber(rule.events_matched_24h)}</div>
                    <div className="text-[10px] text-slate-500">Matched (24h)</div>
                  </div>
                  <span className="text-xs px-2 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-medium">{rule.status}</span>
                </div>
              </div>
              <div className="ml-9 flex items-center gap-2">
                <Server className="w-3 h-3 text-slate-500" />
                <code className="text-xs text-slate-400 bg-slate-900/50 px-2 py-0.5 rounded font-mono border border-slate-700/50">{rule.filter}</code>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { title: 'Ingestion Pipeline', icon: TrendingUp, value: '99.7%', subtitle: 'Pipeline Efficiency', detail: 'Zero-loss event forwarding with automatic retry', color: 'text-emerald-400' },
          { title: 'Parser Coverage', icon: BarChart3, value: `${MOCK_SIEM_CONNECTORS.reduce((s, c) => s + c.parsers_active, 0)}`, subtitle: 'Active Parsers', detail: 'Custom and vendor-supplied log parsers', color: 'text-blue-400' },
          { title: 'Correlation Depth', icon: Shield, value: '3.2x', subtitle: 'Enrichment Factor', detail: 'Cross-platform alert correlation and enrichment', color: 'text-amber-400' },
        ].map(({ title, icon: Icon, value, subtitle, detail, color }) => (
          <div key={title} className="bg-slate-800/40 rounded-xl border border-slate-700/50 p-5">
