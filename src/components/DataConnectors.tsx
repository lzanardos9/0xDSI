...
import { Database, Cloud, Server, Code, Eye, Network, AlertTriangle, CheckCircle, TrendingUp, Layers, FileCode, GitBranch, FileText, Brain, BarChart3, BookOpen, Cpu, Wand2, Radio, Activity, GitMerge, Workflow } from 'lucide-react';
...
import EdgeDeploymentsTab from './connectors/EdgeDeploymentsTab';
import FleetTelemetryDashboard from './connectors/FleetTelemetryDashboard';
import DNAVersionManager from './connectors/DNAVersionManager';
import EdgeToLakePipeline from './connectors/EdgeToLakePipeline';
...
export default function DataConnectors() {
  const [activeTab, setActiveTab] = useState<'overview' | 'catalog' | 'dpi' | 'bytecode' | 'network' | 'cloud' | 'siem' | 'ai-analysis' | 'version-agent' | 'vibe-builder' | 'edge-mesh' | 'fleet-telemetry' | 'dna-versions' | 'edge-to-lake'>('overview');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-800/50 rounded-xl border border-slate-700/50">
        <div className="border-b border-slate-700/50">
          <div className="flex space-x-1 p-2 overflow-x-auto">
            {[
              ...
              { id: 'edge-mesh', label: 'Edge Mesh', icon: Radio },
              { id: 'fleet-telemetry', label: 'Fleet Telemetry', icon: Activity },
              { id: 'dna-versions', label: 'DNA Versions', icon: GitMerge },
              { id: 'edge-to-lake', label: 'Edge → Lake', icon: Workflow }
            ].map((tab) => {
              const Icon = tab.icon;
              return (
                <button
        ...
        <div className="p-6">
          ...
          {activeTab === 'edge-mesh' && <EdgeDeploymentsTab />}
          {activeTab === 'fleet-telemetry' && <FleetTelemetryDashboard />}
          {activeTab === 'dna-versions' && <DNAVersionManager />}
          {activeTab === 'edge-to-lake' && <EdgeToLakePipeline />}
        </div>
      </div>
    </div>
  );
