...
type TabId = 'live' | 'connected' | 'datalink' | 'enterprise' | 'webapi' | 'ground' | 'gnss' | 'incidents' | 'research';
...
interface LiveAircraft {
  ...
  vel: number | null; heading: number | null; onGround: boolean; squawk: string | null;
}
interface LiveThreat {
  icao24: string; callsign: string; originCountry: string; category: string;
  severity: 'critical' | 'high' | 'medium' | 'low'; squawk: string | null;
  latitude: number | null; longitude: number | null; altitude: number | null;
  velocity: number | null; verticalRate: number | null; detail: string;
}
interface RecentEvent {
  icao24: string; callsign: string | null; origin_country: string | null;
  category: string; severity: string; squawk: string | null;
  latitude: number | null; longitude: number | null; altitude_m: number | null;
  detail: string; detected_at: string;
}
interface LiveResponse {
  ...
  feedTime: number;
  globalEmergencies: number;
  stats: { total: number; airborne: number; onGround: number; threats: number };
...
function LiveSky() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${base}/functions/v1/aviation-live?region=${region}`, {
        ...
        else { setError(null); setData(json); setUpdatedAt(new Date()); }
      } catch (e) {
  ...
  // Plot aircraft + threats on a bbox-projected radar map
  useEffect(() => {
    ...
    const pad = 10;
    const projX = (lon: number) => pad + ((lon - lomin) / (lomax - lomin)) * (W - pad * 2);
    ...
    for (const a of data.aircraft) {
      ...
      const x = projX(a.lon), y = projY(a.lat);
      ctx.beginPath(); ctx.arc(x, y, a.onGround ? 1.2 : 1.8, 0, Math.PI * 2);
      ctx.fillStyle = a.onGround ? 'rgba(100,116,139,0.5)' : 'rgba(56,189,248,0.75)';
      ctx.fill();
    }
    // threats highlighted
    for (const t of data.threats) {
      if (t.latitude == null || t.longitude == null) continue;
      const x = projX(t.longitude), y = projY(t.latitude);
      const color = t.severity === 'critical' ? '#EF4444' : t.severity === 'high' ? '#F97316' : '#F59E0B';
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-cyan-400 font-mono font-bold">LIVE ADS-B FEED</span>
          <span className="px-2 py-0.5 text-[10px] rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{data?.source || 'adsb.lol'} · open</span>
          {!!data?.globalEmergencies && (
            <span className="px-2 py-0.5 text-[10px] rounded-full bg-red-500/10 text-red-400 border border-red-500/20">{data.globalEmergencies} global emergency squawks</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          {REGIONS.map((r) => (
            <button key={r.id} onClick={() => setRegion(r.id)}
              className={`px-3 py-1 text-[11px] rounded-lg border transition-colors ${region === r.id ? 'bg-cyan-500/15 text-cyan-300 border-cyan-500/30' : 'bg-[#0b0f1e] text-slate-400 border-[#1e293b] hover:text-slate-200'}`}>
              {r.label}
            </button>
          ))}
        </div>
      ...
      {error && (
        <div className="flex items-start gap-2 bg-amber-500/5 border border-amber-500/20 rounded-xl p-3">
          ...
          <p className="text-[11px] text-amber-200/80 leading-relaxed">
            Live feed is momentarily unavailable ({error}). This panel pulls the open ADS-B feed and retries automatically every 20 seconds.
          </p>
        </div>
      )}
      ...
      {loading && !data && (
        <div className="flex items-center justify-center gap-2 py-16 text-slate-500 text-xs">
          <RefreshCw size={14} className="animate-spin" />Connecting to live flight feed…
        </div>
      )}
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: 'Aircraft Tracked', value: data.stats.total.toLocaleString(), color: 'text-cyan-400', icon: Plane },
              { label: 'Airborne', value: data.stats.airborne.toLocaleString(), color: 'text-sky-400', icon: Activity },
              { label: 'On Ground', value: data.stats.onGround.toLocaleString(), color: 'text-slate-300', icon: MapPin },
              { label: 'Potential Threats', value: data.stats.threats.toString(), color: data.stats.threats ? 'text-red-400' : 'text-emerald-400', icon: AlertTriangle },
            ].map((s, i) => { const I = s.icon; return (
              <div key={i} className="bg-[#0b0f1e] border border-[#1e293b] rounded-xl p-3">
                <div className="flex items-center gap-1.5 text-[10px] text-slate-500 mb-1"><I size={11} />{s.label}</div>
          ...
          <div className="bg-[#0b0f1e] border border-[#1e293b] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-white">Live Traffic Map — {REGIONS.find((r) => r.id === data.region)?.label}</span>
              {updatedAt && <span className="ml-auto text-[10px] text-slate-500 flex items-center gap-1"><Clock size={10} />updated {updatedAt.toLocaleTimeString()}</span>}
            </div>
            <canvas ref={canvasRef} className="w-full rounded-lg" style={{ height: 360 }} />
            <div className="flex items-center gap-4 mt-2 text-[10px] text-slate-500">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-sky-400" />Airborne</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-slate-500" />On ground</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-400" />Potential threat</span>
              <span className="ml-auto">Showing up to 500 of {data.stats.total.toLocaleString()} aircraft</span>
            </div>
          </div>

          {/* live threats */}
          <div className="flex items-center gap-2"><AlertTriangle size={14} className="text-red-400" /><span className="text-xs text-red-400 font-mono font-bold">LIVE POTENTIAL THREATS ({data.threats.length})</span></div>
          {data.threats.length === 0 && (
            <div className="bg-[#0b0f1e] border border-[#1e293b] rounded-xl p-4 text-xs text-slate-400 flex items-center gap-2">
              <Shield size={14} className="text-emerald-400" />No emergency squawks or descent anomalies in this region right now. The map keeps scanning.
            </div>
          )}
          {data.threats.slice(0, 12).map((t, i) => (
            <div key={t.icao24 + i} className="bg-[#0b0f1e] border border-[#1e293b] rounded-xl p-3">
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`px-2 py-0.5 text-[10px] rounded-full border ${sevBadge(t.severity)}`}>{t.severity.toUpperCase()}</span>
                <span className="px-2 py-0.5 text-[10px] rounded bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 font-mono">{catLabel[t.category] || t.category}</span>
                <span className="text-sm font-semibold text-white font-mono">{t.callsign}</span>
                <span className="text-[10px] text-slate-500">{t.originCountry}</span>
              </div>
              <p className="text-xs text-slate-400 mt-1 leading-relaxed">{t.detail}</p>
              <div className="flex items-center gap-4 mt-1.5 text-[10px] text-slate-500 flex-wrap font-mono">
                <span>ICAO {t.icao24}</span>
                {t.squawk && <span>SQ {t.squawk}</span>}
                {t.latitude != null && t.longitude != null && <span className="flex items-center gap-1"><MapPin size={10} />{t.latitude.toFixed(2)}, {t.longitude.toFixed(2)}</span>}
                {t.altitude != null && <span>{Math.round(t.altitude)} m</span>}
                {t.velocity != null && <span>{Math.round(t.velocity)} m/s</span>}
              </div>
            </div>
          ))}
          ...
          {data.recent.length > 0 && (
            <>
              <div className="flex items-center gap-2 pt-1"><Clock size={14} className="text-slate-400" /><span className="text-xs text-slate-400 font-mono font-bold">RECENT DETECTED EVENTS (SAVED HISTORY)</span></div>
              <div className="bg-[#0b0f1e] border border-[#1e293b] rounded-xl divide-y divide-[#1e293b]">
                {data.recent.map((r, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 text-[11px]">
                    <span className={`w-2 h-2 rounded-full ${r.severity === 'critical' ? 'bg-red-400' : r.severity === 'high' ? 'bg-orange-400' : 'bg-amber-400'}`} />
                    <span className="font-mono text-slate-300 w-20 shrink-0">{r.callsign || r.icao24}</span>
                    <span className="text-cyan-400 shrink-0">{catLabel[r.category] || r.category}</span>
                    <span className="text-slate-500 truncate flex-1">{r.detail}</span>
                    <span className="text-slate-600 shrink-0">{new Date(r.detected_at).toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </>
