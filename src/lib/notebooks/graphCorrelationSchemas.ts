...
TBLPROPERTIES ('delta.autoOptimize.optimizeWrite' = 'true');`
    ...
    {
      type: 'code',
      content: `# Cell 4: Multi-Source Bronze Event Ingestion via Auto Loader
# Production-grade ingestion from cloud storage landing zones using
# Databricks Auto Loader (cloudFiles). Each source type lands as JSON
# in its own subdirectory under the configurable landing path.

# --- Configurable widget parameters ---
dbutils.widgets.text("source_landing_path", "abfss://raw@datalake.dfs.core.windows.net/siem/landing", "Landing zone root path")
dbutils.widgets.text("checkpoint_path", "abfss://checkpoints@datalake.dfs.core.windows.net/siem/bronze_checkpoints", "Checkpoint root path")
dbutils.widgets.text("bronze_table", f"{CATALOG}.{SCHEMA_BRONZE}.bronze_events", "Target bronze table")
dbutils.widgets.text("max_files_per_trigger", "1000", "Max files per micro-batch")
dbutils.widgets.text("schema_hints", "event_id STRING, timestamp STRING, type STRING", "cloudFiles schema hints")

SOURCE_LANDING_PATH = dbutils.widgets.get("source_landing_path")
CHECKPOINT_PATH = dbutils.widgets.get("checkpoint_path")
BRONZE_TABLE = dbutils.widgets.get("bronze_table")
MAX_FILES = dbutils.widgets.get("max_files_per_trigger")

# --- Reference constants for downstream enrichment ---
...
# All source types expected in the landing zone subdirectories
SOURCE_TYPES = [
    "iam", "vpn", "edr", "dns", "network",
    "cloud_audit", "email", "threat_intel", "honeypot",
]
...
# --- Auto Loader ingestion per source type ---
# Each source writes JSON files to: {SOURCE_LANDING_PATH}/{source_type}/
# Auto Loader tracks new files via its checkpoint and ingests incrementally.

def ingest_source(source_type):
    """Ingest a single source type from its landing zone subdirectory."""
    landing_path = f"{SOURCE_LANDING_PATH}/{source_type}"
    checkpoint = f"{CHECKPOINT_PATH}/{source_type}"
    ...
    bronze_df = (
        raw_df.select(
            ...
            F.coalesce(
                F.col("source_system"),
                F.lit(f"{source_type}-collector-01")
            ).alias("source_system"),
            F.lit(source_type).alias("source_type"),
            F.coalesce(
                F.col("timestamp"),
                F.col("event_time"),
                F.col("event_time_raw")
            ).cast("string").alias("event_time_raw"),
            F.to_json(F.struct("*")).alias("raw_payload"),
            F.lit("json").alias("raw_format"),
            F.coalesce(F.col("schema_version"), F.lit(1)).cast("integer").alias("schema_version"),
            F.lit(source_type == "honeypot").alias("is_deception"),
        )
    )
...
# --- Launch ingestion for all source types ---
print(f"Starting Auto Loader ingestion from: {SOURCE_LANDING_PATH}")
...
for source_type in SOURCE_TYPES:
    try:
        q = ingest_source(source_type)
        queries[source_type] = q
        print(f"  [{source_type}] Stream started (id={q.id})")
    except Exception as e:
        print(f"  [{source_type}] WARNING - No data found or error: {e}")

# --- Wait for all availableNow triggers to complete ---
for source_type, q in queries.items():
    try:
        q.awaitTermination()
        progress = q.lastProgress
        num_input_rows = progress.get("numInputRows", 0) if progress else "unknown"
        print(f"  [{source_type}] Completed - rows ingested: {num_input_rows}")
    except Exception as e:
        print(f"  [{source_type}] Stream error: {e}")
...
display(df_bronze.groupBy("source_type").agg(
    ...
    F.min("ingestion_time").alias("earliest_ingestion"),
    F.max("ingestion_time").alias("latest_ingestion"),
).orderBy(F.desc("count")))

total = df_bronze.count()
print(f"Total bronze events in table: {total:,}")`
    },
    {
      type: 'code',
