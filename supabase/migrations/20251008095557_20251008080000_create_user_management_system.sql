...
BEGIN
  ...
  ALTER TABLE user_profiles DROP CONSTRAINT IF EXISTS user_profiles_security_clearance_check;
  ALTER TABLE user_profiles ADD CONSTRAINT user_profiles_security_clearance_check
...
ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS account_approved_by uuid;
...
ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS supervisor_id uuid;
...
DROP POLICY IF EXISTS "Anyone can view role definitions" ON user_roles;
CREATE POLICY "Anyone can view role definitions"
  ON user_roles FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Only admins and CISO can modify roles" ON user_roles;
...
DROP POLICY IF EXISTS "Users can view own clearances" ON user_clearances;
...
DROP POLICY IF EXISTS "Only authorized personnel can grant clearances" ON user_clearances;
...
DROP POLICY IF EXISTS "Only authorized personnel can modify clearances" ON user_clearances;
...
DROP POLICY IF EXISTS "Admins and auditors can view audit logs" ON user_audit_log;
...
DROP POLICY IF EXISTS "Anyone can view access control matrix" ON access_control_matrix;
CREATE POLICY "Anyone can view access control matrix"
  ON access_control_matrix FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Only admins can modify access control matrix" ON access_control_matrix;
