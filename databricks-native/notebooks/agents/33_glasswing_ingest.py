...
spark.createDataFrame(scan_row, schema=scan_run_schema).write.mode("append").saveAsTable("glasswing_scan_runs")
...
if input_path:
    ...
    landing_path = f"/Volumes/{catalog}/{schema}/landing/glasswing"
    try:
...
if findings:
    ...
    spark.sql("""
        MERGE INTO glasswing_findings AS target
...
if exploit_chains:
    ...
    chains_df.write.mode("append").saveAsTable("glasswing_exploit_chains")
...
spark.sql(f"""
    ...
    SET total_findings = {len(findings)},
        status = 'ingested',
        hunt_agents_count = {len(set(f.get('hunt_agent_id', '') for f in findings))}
...
spark.sql(f"""
    ...
    USING (SELECT
        'glasswing_ingest' as agent_id,
        current_timestamp() as last_heartbeat,
        'running' as status,
        {len(findings)} as events_processed,
        {len(exploit_chains)} as alerts_generated
    ) AS source
