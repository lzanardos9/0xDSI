      ...
      <div className="bg-gradient-to-r from-slate-800 to-slate-700 rounded-xl p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 mt-1 text-sm">Passive full-duplex traffic capture across all network segments</p>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                  tap.status === 'capturing' ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/30' : 'bg-amber-500/10 text-amber-300 border border-amber-500/30'
                }`}>{tap.status}</span>
              </div>
              <span className="text-xs text-slate-500">{tap.mode}</span>
            </div>
            <div className="text-sm font-semibold text-white mb-1">{tap.name}</div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-slate-900/50 rounded-lg p-2 border border-slate-700/50">
                ...
                <div className="font-bold text-slate-200">{formatNumber(tap.packets_captured)}</div>
              </div>
              <div className="bg-slate-900/50 rounded-lg p-2 border border-slate-700/50">
                <div className="text-slate-500">Throughput</div>
                <div className="font-bold text-cyan-400">{tap.avg_throughput_gbps} Gbps</div>
              </div>
            </div>
          ...
          <div className="space-y-3">
            {PROTOCOL_DISTRIBUTION.map((p) => (
              <div key={p.protocol} className="flex items-center gap-3">
                <div className="w-24 text-xs font-medium text-slate-300">{p.protocol}</div>
        ...
        <div className="bg-slate-800/40 rounded-xl border border-slate-700/50 p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className="bg-slate-900/50 rounded-lg p-3 border border-slate-700/50 flex items-center gap-3">
          ...
          <table className="w-full">
            <thead className="border-b border-slate-700/50 bg-slate-900/30">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">TAP Source</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-700/30">
              {RECENT_CAPTURES.map((cap, i) => (
                <tr key={i} className={`hover:bg-slate-700/20 ${cap.alert ? 'bg-red-500/5' : ''}`}>
                  <td className="px-4 py-2.5 text-xs font-mono text-slate-400">{cap.time}</td>
                  <td className="px-4 py-2.5 text-xs font-mono text-cyan-400">{cap.src}</td>
                  <td className="px-4 py-2.5 text-xs font-mono text-slate-200">{cap.dst}</td>
                  <td className="px-4 py-2.5">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/30 font-medium">{cap.protocol}</span>
                  </td>
                  <td className="px-4 py-2.5 text-xs text-slate-400">{cap.size}B</td>
